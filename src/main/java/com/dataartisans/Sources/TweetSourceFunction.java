  ...
  public void run(SourceContext ctx) throws Exception {
    ...
    while (running) {
      ...
      for(int i=0; i < (injectDuplicates ? NUM_DUPLICATES : 1); i++) {
        TweetImpression tweetImpression = new TweetImpression((Math.abs(random.nextLong()) % NUM_TWEETS) + 1);
        System.out.println("Generated tweet id - " + tweetImpression.getTweetId() );
        ctx.collect( tweetImpression );
      }
    }
  }
