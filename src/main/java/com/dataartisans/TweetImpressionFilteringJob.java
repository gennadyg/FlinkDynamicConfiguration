...
import com.dataartisans.Sources.*;
...
public class TweetImpressionFilteringJob {
  ...
  public static void main(String[] args) throws Exception {
    ...
    DataStream<CustomerImpression> subscriptionStream = tweetStream
      .connect(filterUpdateStream)
      .flatMap(new TweetSubscriptionFilterFunction());

    // Create a seperate sink for each customer
    DataStreamSink<CustomerImpression>[] customerSinks = setupCustomerSinks( subscriptionStream );
  ...
  private static MapFunction<String, TweetSubscription> stringToTweetSubscription() {
    return new MapFunction<String, TweetSubscription>() {
      ...
      public TweetSubscription map(String value) throws Exception {
        ...
        LOG.info("Customer[" + splits[0] + "] asking to subscribe to tweets - " + splits[1] + " with time format - " + splits[2]);
        return new TweetSubscription(new Customer(splits[0]), Long.valueOf(splits[1]), splits[2]);
      }
    };
  }
